class Cdn {
    ...
    encode(path: string, hash = this.hash) {
        switch (this.host) {
            case 'jsdelivr':
                return `${this.protocol}://fastly.jsdelivr.net/gh/IcedDog/Bilibili-Old${hash ? `@${hash}` : ''}${path}`;
            default:
                return `${this.protocol}://github.com/IcedDog/Bilibili-Old/raw/${hash || 'master'}${path}`;
        }
    }
    /** 更新默认值 */
